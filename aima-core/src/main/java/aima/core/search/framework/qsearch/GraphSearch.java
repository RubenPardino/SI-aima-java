...
public class GraphSearch<S, A> extends TreeSearch<S, A> {
	...
	protected void addToFrontier(Node<S, A> node) {
		if (!reached.containsKey(node.getState()) || reached.get(node.getState()).getPathCost() > node.getPathCost()) {
			frontier.add(node);
			updateMetrics(frontier.size());
			
			if (explored.contains(node.getState()))
				metrics.incrementInt(METRIC_NODES_EXPANDED_REINSERTED_IN_FRONTIER); // El estado del nodo ya fue explorado
			else if (reached.containsKey(node.getState()))
				metrics.incrementInt(METRIC_NODES_DUPLICATED_IN_FRONTIER); // Ya hay un nodo con ese estado en frontier
