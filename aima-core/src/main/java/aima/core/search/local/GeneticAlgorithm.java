...
public class GeneticAlgorithm<A> {
	...
	protected Individual<A> randomSelection(List<Individual<A>> population, FitnessFunction<A> fitnessFn) {
		...
		double menorFitness = fitnessFn.apply(population.get(0));
		for (int i = 0; i < population.size(); i++) {
			fValues[i] = fitnessFn.apply(population.get(i));
			if (menorFitness > fitnessFn.apply(population.get(i)))
	...
	protected Individual<A> reproduceOX(Individual<A> x, Individual<A> y) {    // REPASALO, IGUAL FUNCIONA BIEN, CREO QUE SI
		...
		int a1 = randomOffset(individualLength);
		int a2 = randomOffset(individualLength);
		// return APPEND(SUBSTRING(x, 1, c), SUBSTRING(y, c+1, n))
		List<A> childRepresentation = new ArrayList<A>();
		
		for (int i = 0 ; i < x.getRepresentation().size(); i++)
			childRepresentation.add(null);
		
		if (a1 != a2) {
			int puntero = -1;
			for (int i = a1; i < x.getRepresentation().size() && puntero == -1; i++) {
				if (i == a2)
					puntero = a2;
				else {
					childRepresentation.set(i, x.getRepresentation().get(i));
					if (i == x.getRepresentation().size()-1)
						i = -1;
				}
			}
			
			for (int i = 0; i < y.getRepresentation().size() && puntero != a1; i++) {
				if (!childRepresentation.contains(y.getRepresentation().get(i))) {
					childRepresentation.set(puntero, y.getRepresentation().get(i));
					puntero++;
					if (puntero >= y.getRepresentation().size())
						puntero = 0;
				}
				if (i == y.getRepresentation().size()-1)
					i = -1;
			}
		} else {
			childRepresentation.clear();
			childRepresentation.addAll(x.getRepresentation());
		}
