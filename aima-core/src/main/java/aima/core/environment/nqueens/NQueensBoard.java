...
public class NQueensBoard {
	...
	public NQueensBoard(int size, Config config) {
		...
		if (config == Config.QUEENS_IN_FIRST_ROW) {
			...
			for (int col = 0; col < size; col++) {
				do {
					row = r.nextInt(size);
				} while (isSquareHorizontallyAttacked(col, row));
				
				addQueenAt(new XYLocation(col, row));
			}
		}
	}
	...
	public int getNumberOfAttackedQueens() {
		int attacked = 0;
		ArrayList<XYLocation> positions = (ArrayList<XYLocation>) getQueenPositions();
		for (XYLocation position : positions) {
	...
	public int getMaximumNumberOfQueensAlignedMinusOne() {
		...
		ArrayList<XYLocation> positions = (ArrayList<XYLocation>) getQueenPositions();
		for (XYLocation position : positions) {
			int horizontal = numberOfHorizontalAttacksOn(position.getX(), position.getY());
			int vertical = numberOfVerticalAttacksOn(position.getX(), position.getY());
			int diagonal = numberOfDiagonalAttacksOn(position.getX(), position.getY());
			int mayor = Math.max(horizontal, Math.max(vertical, diagonal));
	...
	public double getHeuristicProbabilisticEstimationOfSolution() {
		int nonAttackedSquares = 0;
		int attackedSquares = 0;
		for (int i = 0; i < getSize(); i++) {
			for (int j = 0; j < getSize(); j++) {
				if (isSquareUnderAttack(new XYLocation(i, j)))
						attackedSquares++;
			}
		}
		
		nonAttackedSquares = getSize() * getSize() - attackedSquares;
		
		return ( (double) (getSize() - getNumberOfQueensOnBoard())) / ( (double) nonAttackedSquares / (double) getSize() * (getSize() - getNumberOfQueensOnBoard()) );
	}
